...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (userInput !== captcha) {
      alert("Incorrect CAPTCHA");
      generateCaptcha();
      setUserInput("");  // Clear the input after incorrect CAPTCHA
      ...
      <div className="relative flex flex-col md:flex-row w-full md:w-4/5 bg-white rounded-lg shadow-lg gap-6 p-4">
        ...
        <div className="w-full md:w-2/5 bg-white p-4 ml-10 md:p-6 mb-4 mt-8 rounded-lg">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex justify-center w-full">
              <button
                className={`w-full md:w-[170px] md:h-[50px] bg-blue-900 text-white rounded-md px-3 py-2 mt-6 hover:bg-blue-700 transition flex items-center justify-center text-xl relative overflow-hidden ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
                type="submit"
